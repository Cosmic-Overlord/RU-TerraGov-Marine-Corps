...
/mob/living/carbon/xenomorph/proc/caste_evolution_checks(new_mob_type, castepick, regression = FALSE)
	...
	if(CHECK_BITFIELD(new_caste_flags, CASTE_LEADER_TYPE))
		...
		if(xenojob.required_playtime_remaining(client))
			// to_chat(src, span_warning("[get_exp_format(xenojob.required_playtime_remaining(client))] as [xenojob.get_exp_req_type()] required to play queen like roles.")) // ORIGINAL
			to_chat(src, span_warning("[get_exp_format(xenojob.required_playtime_remaining(client))] as [xenojob.exp_type] required to play the queen role.")) // RUTGMC EDIT
			return FALSE
